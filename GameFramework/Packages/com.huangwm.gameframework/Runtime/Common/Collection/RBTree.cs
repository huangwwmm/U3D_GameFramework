...
{
    public enum TreeAccessMethod
    {
        KEY_SEARCH_AND_INDEX = 1,
        INDEX_ONLY = 2,
    }
    /// <summary>
    /// Copy from .net System.Data.RBTree
    /// </summary>
    ...
    public abstract class RBTree<K> : IEnumerable
    {
            ...
            while (pageTableMapPos < _pageTableMap.Length)
            {
                if (((UInt32)_pageTableMap[pageTableMapPos]) < 0xFFFFFFFF)
                {
                    ...
                    while ((pageSegmentMap ^ (0xFFFFFFFF)) != 0)         //atleast one "0" is there (same as <0xFFFFFFFF)
                    {
                        ...
                        {
                            throw new Exception("PagePositionInSlotInUse");
                        }
        ...
        public int Count
        {
            get
            {
                MDebug.Assert(_inUseNodeCount - 1 == SubTreeSize(root)
                    , "RBTree"
                    , "count mismatch");
        ...
        private int GetNewNode(K key)
        {
            ...
            if (slotId == -1)
            {
                throw new Exception("NoFreeSlots");
            }

            // NodeId: Upper 16 bits pageId, lower bits slotId
            page.Slots[slotId].selfId = (int)(((UInt32)page.PageId) << 16) | slotId;
            MDebug.Assert(page.Slots[slotId].leftId == NIL, "RBTree", "node not cleared");
            MDebug.Assert(page.Slots[slotId].rightId == NIL, "RBTree", "node not cleared");
            MDebug.Assert(page.Slots[slotId].parentId == NIL, "RBTree", "node not cleared");
            MDebug.Assert(page.Slots[slotId].nextId == NIL, "RBTree", "node not cleared");
            page.Slots[slotId].subTreeSize = 1;     // new Nodes have size 1.
            page.Slots[slotId].keyOfNode = key;
            MDebug.Assert(page.Slots[slotId].nodeColor == NodeColor.red, "RBTree", "node not cleared");
            ...
            {   // test for satellite branch
                ...
                {   // find first node of satellite branch
                    ...
                    {   // satellite branch has satellite branch - very bad
                        throw new Exception("NestedSatelliteTreeEnumerator");
                    }
        ...
        {
            MDebug.Assert(NIL != x_id, "RBTree", "nil left");
            MDebug.Assert(NIL != z_id, "RBTree", "nil right");
        ...
        private int RBInsert(int root_id, int x_id, int mainTreeNodeID, int position, bool append)
        {
            ...
            {
                MDebug.Assert(-1 == position, "RBTree", "KEY_SEARCH_AND_INDEX with bad position");
                while (z_id != NIL)  // in-order traverse and find node with a NILL left or right child
                {
                    ...
                    {
#if VerifySort
                        MDebug.Assert((NIL == Left(z_id)) || (0 > Compare(root_id, Left(z_id), z_id)), "RBTree", "Left is not left");
#endif
                        z_id = Left(z_id);
                    }
                    else if (c > 0)
                    {
#if VerifySort
                        MDebug.Assert((NIL == Right(z_id)) || (0 < Compare(root_id, Right(z_id), z_id)), "RBTree", "Right is not right");
#endif
                        z_id = Right(z_id);
                    }
                    else
                    {
                        ...
                        if (root_id != NIL)
                        {
                            throw new Exception("InvalidStateinInsert");
                        }
                        if (Next(z_id) != NIL)
                        {
            ...
            {
                throw new Exception("UnsupportedAccessMethod1");
            }
            ...
            else
            {
                ...
                {
                    throw new Exception("UnsupportedAccessMethod2");
                }
        ...
        public int RBDelete(int z_id)
        {
            ...
            MDebug.Assert(_accessMethod == TreeAccessMethod.INDEX_ONLY, "not expecting anything else");
        ...
        private int RBDeleteX(int root_id, int z_id, int mainTreeNodeID)
        {
            ...
            {
                throw new Exception("InvalidNextSizeInDelete");
            }
            ...
            if (Next(mNode) != NIL)
            {
                ...
                {
                    throw new Exception("InvalidStateinDelete");
                }
            ...
            if (isCase3)
            {
                ...
                {
                    throw new Exception("InvalidNodeSizeinDelete");
                }
                _inUseSatelliteTreeCount--;
            ...
            else if (Next(mNode) != NIL)
            {
                ...
                { //if mNode being deleted, its OK for root_id (it should be) NIL.
                    throw new Exception("InvalidStateinEndDelete");
                }
        ...
        {    //x is successor's non nil child or nil if both children are nil
            ...
            while (((root_id == NIL ? root : root_id) != x_id) && color(x_id) == NodeColor.black)
            {
                ...
                if ((x_id != NIL && x_id == Left(Parent(x_id))) || (x_id == NIL && Left(px_id) == NIL))
                {
                    ...
                    if (w_id == NIL)
                    {
                        throw new Exception("RBDeleteFixup");
                    }
                ...
                {  //x is a right child or it is NIL
                    ...
                    {   // x_id is y's (the spliced node) sole non-NIL child or NIL if y had no children
                        ...
                        else
                        {
                            ...
                            if (w_id == NIL)
                            {
                                throw new Exception("CannotRotateInvalidsuccessorNodeinDelete");
                            }
                        }
                    }
        ...
        private int SearchSubTree(int root_id, K key)
        {
            ...
            {
                throw new Exception("UnsupportedAccessMethodInNonNillRootSubtree");
            }
            ...
            while (x_id != NIL)
            {
                ...
                {
#if VerifySort
                    MDebug.Assert((NIL == Left(x_id)) || (0 > Compare(root_id, Left(x_id), x_id)), "Search duplicate Left is not left");
#endif
                    x_id = Left(x_id);
                }
                else
                {
#if VerifySort
                    MDebug.Assert((NIL == Right(x_id)) || (0 < Compare(root_id, Right(x_id), x_id)), "Search duplicate Right is not right");
#endif
                    x_id = Right(x_id);
                }
            }
        ...
        {   // for performance reasons, written as a while loop instead of a recursive method
            ...
            while (x_id != NIL)
            {
                ...
                {
#if VerifySort
                    MDebug.Assert((NIL == Left(x_id)) || (0 > Compare(NIL, Left(x_id), x_id)), "Search Left is not left");
#endif
                    x_id = Left(x_id);
                }
                else
                {
#if VerifySort
                    MDebug.Assert((NIL == Right(x_id)) || (0 < Compare(NIL, Right(x_id), x_id)), "Search Right is not right");
#endif
                    x_id = Right(x_id);
                }
            }
        ...
        {
            MDebug.Assert(NIL != node, "GetIndexByNode(NIL)");
        ...
        {
#if VerifyIndex
            MDebug.Assert(NIL != nodeId, "ComputeIndexByNode(NIL)");
#endif
            int myRank = SubTreeSize(Left(nodeId));
            while (nodeId != NIL)
            {
#if VerifyIndex && VerifyPath
                MDebug.Assert(NIL == Next(nodeId), "Next not NIL");
#endif
        ...
        {
#if VerifyIndex
            MDebug.Assert(NIL != nodeId, "ComputeIndexWithSatelliteByNode(NIL)");
#endif
            int myRank = SubTreeSize(Left(nodeId));
            while (nodeId != NIL)
            {
        ...
        private NodePath GetNodeByIndex(int userIndex)
        {
            ...
            if (x_id == NIL)
            {
                ...
                {
                    throw new IndexOutOfRangeException($"RowOutOfRange{userIndex}");
                }
                else
                {
                    throw new Exception("IndexOutOFRangeinGetNodeByIndex");
                }
            }
        ...
        private int ComputeNodeByIndex(int x_id, int index)
        {
            while (x_id != NIL)
            {
                MDebug.Assert(NIL == Next(x_id), "has unexpected satellite tree");
        ...
        public void CopyTo(Array array, int index)
        {
            ...
            {
                throw new Exception("InvalidOffsetLength");
            }
        ...
        {
            MDebug.Assert(nodeId != NIL, " in SetParent  nodeId == NIL");
            /*
            TreePage page = _pageTable[nodeId >> 16];
            int slotIndex = nodeId & 0xFFFF;
        ...
        {
            MDebug.Assert(nodeId != NIL, " in SetColor  nodeId == NIL");
            /*
            TreePage page = _pageTable[nodeId >> 16];
            int slotIndex = nodeId & 0xFFFF;
        ...
        {
            int myCorrectSize = SubTreeSize(Left(nodeId)) + SubTreeSize(Right(nodeId)) + (Next(nodeId) == NIL ? 1 : SubTreeSize(Next(nodeId)));
            MDebug.Assert(myCorrectSize == size, "VerifySize");
        }
        ...
        private struct NodePath
        {
            ...
            {
                MDebug.Assert(null != tree, "null tree");
                MDebug.Assert((NIL == NodeID && NIL == MainTreeNodeID) || (NIL != NodeID), "MainTreeNodeID is not NIL");

                if (NIL != MainTreeNodeID)
                {
                    MDebug.Assert(NIL != tree.Next(MainTreeNodeID), "MainTreeNodeID should have a Next");
                    int node = MainTreeNodeID;
                    while (NIL != tree.Parent(node))
                    {
                        node = tree.Parent(node);
                    }
                    MDebug.Assert(tree.root == node, "MainTreeNodeID parent change doesn't align");
                }
                if (NIL != NodeID)
                {
                    MDebug.Assert(NIL == tree.Next(NodeID), "NodeID should not have a Next");
                    int node = NodeID;
                    if (NIL == MainTreeNodeID)
                    {
                        while (NIL != tree.Parent(node))
                    ...
                    else
                    {
                        while (NIL != tree.Parent(node))
                        {
                            MDebug.Assert(NIL == tree.Next(node), "duplicate node should not have a next");
                            node = tree.Parent(node);
                        }
                    }
                    MDebug.Assert((NIL == MainTreeNodeID && tree.root == node) ||
                ...
                if (_inUseCount < Slots.Length)
                {
                    ...
                    while (segmentPos < SlotMap.Length)
                    {
                        if (((UInt32)SlotMap[segmentPos]) < 0xFFFFFFFF)
                        {
                            ...
                            MDebug.Assert((SlotMap[segmentPos] & freeSlot) == 0, "Slot position segment[segmentPos ]: [freeSlot] is in use. Expected to be empty");
        ...
        internal struct RBTreeEnumerator : System.Collections.Generic.IEnumerator<K>, System.Collections.IEnumerator
        {
            ...
            public bool MoveNext()
            {
                ...
                {
                    throw new Exception("EnumeratorModified");
                }
